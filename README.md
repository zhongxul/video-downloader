...
- 支持新 UI 三页主导航：下载、资源库、我的。
- 支持下载队列、失败重试、资源库管理、已完成管理。
- 支持下载前后双阶段校验，自动拦截空文件、网页内容和异常文件。
- 支持同名文件自动重命名：`标题.mp4`、`标题(1).mp4`、`标题(2).mp4`。
- 资源库统一承接“进行中 / 已完成 / 解析记录”，已完成不再保留独立入口。
- 资源确认页支持多资源横向切换；视频仅在资源确认页预览区域点击后原地播放。
- 我的页接入 X Cookie 设置入口。
...
- `pencil-new.pen` 是 2026-05-03 新 UI 设计源文件。
