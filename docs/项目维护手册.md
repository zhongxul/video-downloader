...
3. 最后改 `ui/` 提示与交互。
4. 若涉及未来方案，请先确认 `docs/plans/` 是否已真正落地，不要直接按方案假设现状。
...
- 已完成任务管理：`HistoryScreen` + `HistoryViewModel`。
- 解析结果页预览：`ParseResultScreen` + `ParseResultViewModel`。
