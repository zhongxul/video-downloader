# 项目启动说明

> 文档版本：v1.5.0
> 最后更新：2026-05-05

## 1. 环境要求
- Android Studio（最新稳定版）
- JDK 17（可直接使用 Android Studio 自带 JBR）
- Android SDK（项目默认路径或本机已配置路径）
- 真机或模拟器（Android 10+）

## 2. 打开项目
1. 打开 Android Studio
2. 选择 `Open`
3. 打开目录 `D:\Video Downloader`
4. 等待 Gradle Sync 完成

## 3. 终端构建（推荐）
```powershell
.\环境一键配置.ps1
.\\gradlew.bat :app:testDebugUnitTest
.\gradlew.bat :app:assembleDebug
```

产物位置：`app/build/outputs/apk/debug/app-debug.apk`

## 4. 安装到设备
优先方式：
```powershell
.\gradlew.bat :app:installDebug
```

若本机 `adb` 不在 PATH，可直接执行：
```powershell
D:\Android\Sdk\platform-tools\adb.exe devices -l
D:\Android\Sdk\platform-tools\adb.exe install -r .\app\build\outputs\apk\debug\app-debug.apk
```

若出现 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`（签名不一致）：
```powershell
D:\Android\Sdk\platform-tools\adb.exe uninstall com.example.videodownloader
D:\Android\Sdk\platform-tools\adb.exe install .\app\build\outputs\apk\debug\app-debug.apk
```

## 5. 最小验证流程
1. 下载页粘贴抖音分享文案并解析，进入资源确认页。
2. 用一条 `kstore.vip/*.html#...` 链接（如 `bluegay/xgaymv`）解析并下载。
3. 在资源确认页验证缩略图横向切换；缩略图点击只切换选择，不应触发播放。
4. 视频资源点击主展示区播放按钮应进入 App 内播放器；返回后不应残留播放画面。
5. 点击“下载当前”或“下载全部”，观察资源库“进行中”卡片进度。
6. 下载完成后进入“资源库”：
   - 在“进行中”栏验证空状态或真实任务卡片
   - 在“进行中”栏长按卡片应进入管理模式
   - 在“已完成”栏长按卡片应进入管理并选中该条
   - 已完成列表只显示封面，合集项右上角应显示合集标识
   - 已完成详情应按纵向媒体流展示，图片点开 App 内查看器，视频点播放按钮进入 App 内播放器
7. 在“解析记录”栏确认本次解析记录存在，可点击展开摘要，长按进入管理。
8. 进入“我的”页面，确认存在 X Cookie 设置入口和下载完成通知开关。
9. 关闭下载完成通知开关后下载完成不应弹出完成通知；重新开启后恢复。

## 6. 常见提示
- 如果 Android Studio 一直下载 `gradle-*-src.zip`：这是源码包下载，不影响命令行构建与安装。
- 如果 X 解析失败：优先检查 Cookie 是否失效、代理是否可用。
- 如果 kstore 链接失败：优先检查 `auth_key` 是否过期。
- 若出现 `unsupported m3u8 key method`：说明链接使用了非 `AES-128` 加密方式。
- 若 App 内播放器无法播放 m3u8/HLS：优先检查 Media3 依赖、源地址是否过期，以及合并文件是否通过有效性校验。

## 7. X 解析测速与定位
```powershell
D:\Android\Sdk\platform-tools\adb.exe logcat -c
D:\Android\Sdk\platform-tools\adb.exe logcat | findstr WebParserGateway
```

重点观察日志：
- `parseXFast success/fail cost=...`
- `parseXStatusFast branch=... hit=... cost=...`
- `parseTwitterFx mirror=... hit=... cost=...`

## 8. 一键清理
```powershell
.\项目清理.ps1
```

当前 `.gitignore` 已忽略临时截图、调试数据库和 `.claude/` 本机配置；这些文件不应提交。

## 变更记录
- 2026-02-10 v1.0.0：重构启动指引并对齐当时项目结构与常用命令
- 2026-02-10 v1.1.0：补充 m3u8 推荐/预览验证步骤
- 2026-02-14 v1.2.0：补充设备直装与签名冲突处理、m3u8 提速验证步骤
- 2026-02-14 v1.3.0：补充 kstore hash 链接与 AES-128 下载验证、已完成页交互验证
- 2026-02-14 v1.3.1：补充 X 解析测速与日志定位步骤
- 2026-04-26 v1.3.2：补充历史中心单入口验证与测试命令
- 2026-05-03 v1.4.0：对齐新 UI 启动验证流程、资源确认页播放边界、资源库管理与 X Cookie 入口
- 2026-05-05 v1.5.0：补充内置媒体查看/播放、通知开关、资源库悬浮分段、失败集合重试与 X 多媒体验证
